...
# Car Wash Reminder Model
class CarWashReminder(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    vehicle_id = db.Column(db.Integer, db.ForeignKey('vehicle.id'), nullable=False)
    reminder_date = db.Column(db.Date, nullable=False)
    interval_mileage = db.Column(db.Float, nullable=False)
...
def manage_vehicles():
    if request.method == "POST":
        ...
        vehicle = Vehicle(name=name, make=make, model=model, year=year, user_id=current_user.id)
...
@app.route("/maintenance_calendar")
@login_required
def maintenance_calendar():
    # Fetch all events from the database for the logged-in user
    events = [{
        "title": record.service_type,
        "start": record.date.isoformat()
    } for record in MaintenanceRecord.query.filter_by(user_id=current_user.id).all()]

    # Render the HTML template and pass events as context
    return render_template('maintenance_calendar.html', events=events)

@app.route('/log_mileage', methods=['GET', 'POST'])
@login_required
def log_mileage():
    ...
    if request.method == 'POST':
        ...
        new_mileage = Mileage(vehicle_id=vehicle_id, mileage=mileage)
        db.session.add(new_mileage)
        db.session.commit()
        
        flash("Mileage logged successfully!", "success")
        return redirect(url_for('dashboard'))
    
    # Pass vehicles to the template for the user to select
    return render_template('log_mileage.html', vehicles=vehicles)

@app.route('/add_car_wash_reminder', methods=['GET', 'POST'])
@login_required
def add_car_wash_reminder():
    # Assuming you are fetching the vehicle somehow
    vehicle = Vehicle.query.filter_by(user_id=current_user.id).first()  # Adjust to get the right vehicle
    
    if request.method == 'POST':
        # Logic to add car wash reminder
        pass
    
    # Pass the vehicle object to the template
    return render_template('add_car_wash_reminder.html', vehicle=vehicle)
